...
def affine_backward(dout, cache):
  ...
  dw = np.dot(x_N.T, dout)  # /N here we do not divide
...
def batchnorm_backward(dout, cache):
  ...
  one_over_sqrt_var = 1/np.sqrt(sample_var+eps)  # (D,)
  one_over_sqrt_var_to_three = one_over_sqrt_var*one_over_sqrt_var*one_over_sqrt_var
  # MISTAKE! Do not write as -1/2
  dsample_var = -1.0/2*one_over_sqrt_var_to_three*np.sum(dx_normalized*(x-sample_mean), axis=0)
  dsample_mean = -one_over_sqrt_var*np.sum(dx_normalized, axis=0)
  # dsample_var*(-2/N)*np.sum((x-sample_mean), axis=0)
  dx = one_over_sqrt_var*dx_normalized+dsample_var*(2.0/N)*(x-sample_mean)+dsample_mean/N
